"""
python3.12 Scripts/train_model.py -in_dir Dataset -out_dir Models
"""
import numpy as np
import os
import argparse
...
from keras.layers import Input, Conv1D, MaxPooling1D, GlobalAveragePooling1D, Dense, Dropout, BatchNormalization
...
for trainIdx, testIdx in logo.split(X, yEnc, groups):
    ...
    savePath = os.path.join(outPath, f"model_{fold}.keras")
...
np.save(os.path.join(outPath, "cm.npy"), cm)
